...
public class LineClickListener implements PacketListener {
    ...
    public void processQueuedClickEvents() {
        for (QueuedClickEvent event : queuedClickEvents) {
            if (event.lineTracker.getLine().hasClickCallback()
                    && event.lineTracker.canInteract(event.player)
                    && event.lineTracker.isInClickRange(event.player)) {
                event.lineTracker.getLine().onClick(event.player);
            }
        }
        queuedClickEvents.clear();
    }

    public void registerLine(EntityID clickableEntityID, ClickableLineTracker<?> lineTracker) {
